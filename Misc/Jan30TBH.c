...
#pragma config(Motor,  port7,           driveRight2,   tmotorVex393_MC29, openLoop, reversed)
...
task TBHController()
{
	...
	while( true )
	{
		...
		if( TBHRunning )
		{
			...
			//limit drive
			if( TBHDriveR > PID_DRIVE_MAX )
				TBHDriveR = PID_DRIVE_MAX;
			if( TBHDriveR < PID_DRIVE_MIN )
				TBHDriveR = PID_DRIVE_MIN;
			...
			//limit drive
			if( TBHDrive > PID_DRIVE_MAX )
				TBHDrive = PID_DRIVE_MAX;
			if( TBHDrive < PID_DRIVE_MIN )
				TBHDrive = PID_DRIVE_MIN;
...
/* task pidController()
{
	...
	while (true)
	{
		if(pidRunning)
		{
			...
			pidDriveR+= (pid_Kp_right*pidErrorR) + (pid_Ki*pidIntegralR) + (pid_Kd_right * pidDerivativeR);
...
slowDown();
		...
		datalogAddValue( 2, TBHDrive );
		datalogAddValue( 3, TBHDriveR );
		datalogDataGroupEnd();
		wait1Msec(25);
	}
}
